...
* [`src/cljs/studying_interops_on_re_frame/`](src/cljs/studying_interops_on_re_frame/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/studying_interops_on_re_frame/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `studying-interops-on-re-frame` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/studying_interops_on_re_frame/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns studying-interops-on-re-frame.example
  (:require [studying-interops-on-re-frame.config :as config])
